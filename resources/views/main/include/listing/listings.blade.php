                    ...
                    <div class="listsearch-maiwrap box-inside fl-wrap">
                                            ...
                                            <article class="geodir-category-listing fl-wrap">
                                                <div class="geodir-category-img">
                                                    <a href="{{ route('listingDetail', $listing->seflink) }}">
                                                        <img
                                                            src="{{ asset( 'main/images/listing/'. $listing->image ) }}"
                                                            alt="{{ $listing->name }}">
                                                    </a>
                                                ...
                                                <div class="geodir-category-content fl-wrap">
                                                    ...
                                                    <p>Ürün Tipi: @if($listing->type==1) Satılık @else Kiralık @endif
                                                        <span class="fa fa-shopping-bag"></span></p>
                                                    <p>Ürün Kategorisi: <a
                                                            href="{{ route('categoryDetail', $listing->category->seflink) }}">{{ $listing->category->name }} </a>
                                                    </p>
                                                </div>
                                            </article>
                                        </div>
                        ...
                        @if($listings->lastPage()>1)
                            <div class="pagination">
                                <a href="{{ $listings->previousPageUrl() }}" class="prevposts-link"><i
                                        class="fa fa-caret-left"></i></a>
                                @for($i = 1; $i<=$listings->lastPage(); $i++)
                                    <a href="{{ $listings->url($i) }}"
                                       @if($listings->currentPage()==$i) class="current-page" @endif>{{ $i }}</a>
                                @endfor
                                <a href="{{ $listings->nextPageUrl() }}" class="nextposts-link"><i
